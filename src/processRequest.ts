...
import { listFunkos, ListFunkoArgs, formatFunkoData } from "./commands/list.js";
...
process.on("message", (request: RequestType) => {
  ...
  try {
    switch (request.type) {
      ...
      case "list": {
        const listArgs: ListFunkoArgs = { user: request.user };
  ...
  // Enviar la respuesta de vuelta al proceso principal
